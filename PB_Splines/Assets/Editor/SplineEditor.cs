...
public class SplineEditor : Editor
{
    ...
    Vector3 ShowPoint(int pointIndex, int curveIndex, Spline spline, BezierCurve curve, Transform handleTransform, Quaternion handleRotation)
    {
        ...
        {
            if (pointIndex != 0 && curveIndex != 3) return point;
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                {
                    if (curveIndex == spline.curves.Length-1 && spline.Loop) spline.curves[0].angles[0] = newAngle;
                    else if (curveIndex < spline.curves.Length-1) spline.curves[curveIndex + 1].angles[0] = newAngle;
                }
            }
        }
